...
addpath(genpath('/home/silviu/erika-organoid-data/CODE/CURRENT-FOLDER/signal-correlation-master')); % contains meta_k_means_spontactivity.m function
...
%% Meta k means
[allclusters, centroidcorr, dendmem, dunnsinitial] = meta_k_means_spontactivity(F_dff_dec', 'correlation');
...
num_neurons = size(F_dff_dec, 2); % Columns represent neurons
...
for cluster_index = 1:num_clusters
    ...
    cluster_signals = F_dff_dec(:, cluster_neurons); % Extracting signals of these neurons
...
for neuron = 1:num_neurons
    ...
    if length(assigned_clusters) > 1
        ...
        neuron_signal = F_dff_dec(:, neuron)';
...
for i = 1:num_clusters
    if ~isempty(unique_clusters{i})
        ...
        cluster_neurons_signals = F_dff_dec(:, unique_clusters{i});
...
F_reordered = F_dff_dec(:, new_order);
...
% Define cluster boundaries based on allclusters_new
...
if SAVE_FLAG
    ...
    save(full_path, 'allclusters_new', 'neuron_changes', 'remove_columns', 'F_dff_dec', 'allclusters', 'centroidcorr', 'dendmem', 'dunnsinitial');
