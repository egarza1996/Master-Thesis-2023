...
for org_idx = 2:length(organoid_folders) % org1 is faulty
...
for i = 1:num_traces
    ...
    ylabel('Activity');
    xlabel('Frame Number');
    xlim([0 6600]); % CHANGE depending on cond1 (6600) or cond2/3 (12600)
...
frames_of_interest = 1:6600;  % CHANGE: For cond1 (6600) and for cond2/3 (12600)

% Define stimulation points within the range of interest
stimulation_points = 600:200:6600;  
...
% stimulation_interval = 200; % every 10sec (200) for cond1 and every 20sec (400) for cond 2/3
% total_frames = 6600; % end of 10min30sec - total frames of interest
...
frame_rate = 20; % 20 frames per second
stimulation_start = 600; % stimulation starts at 30 seconds
stimulation_interval = 200; % every 10 or 20 seconds
...
% Define constants based on dataset details
frame_rate = 20; % 20 frames per second
window_length = frame_rate * 5; % cond1 is 5sec and cond2/3 is 15sec in frames
delay_after_stimulation = frame_rate * 5; % 5 seconds in frames
stimulation_start = 600; % stimulation starts at 30 seconds
stimulation_interval = 200; % CHANGE every 10 or 20 seconds
total_frames = 6600; % CHANGE total frames of interest
