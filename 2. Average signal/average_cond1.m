...
% Code for condition 1 (1msec every 10sec)
...
for org_idx = 2 % 1:length(organoid_folders)
...
for i = 1:size(F_dff,2)
  ...
  for m = stim_start(1:end)
      ...
      av_neuron(:,k) = f(m-20:m+199); % cond1: 1sec (20frames) before stim, and 10sec after (200frames)
...
figure;
% 9.95 for cond1 (corresponds to stimulus cycle time length)
shadedErrorBar(-1:0.05:9.95, neurons_stim_cycles{50}'*100, {@mean,@std}); % TIME and also multiply by 100 because of % ΔF/F
hold on;
...
% Another way of viewing this with frames instead of time on the x axis...
    % 220 for cond1
% figure;
% shadedErrorBar(1:220, neurons_stim_cycles{50}'*100, {@mean,@std}); % FRAMES
% hold on;
% xlim([0 220]); % ylim([ymin ymax]);
...
figure;
% 9.95 for cond1 (corresponds to stimulus cycle time length)
shadedErrorBar(-1:0.05:9.95, av_neuron'*100, {@mean,@std}); % TIME and also multiply by 100 because of % ΔF/F
hold on;
...
% Another way of viewing this...
    % 220 for cond 1
% figure;
% shadedErrorBar(1:220, av_neuron', {@mean,@std}); % FRAMES
% hold on;
% xlim([0 220]); % ylim([ymin ymax]);
% vline(20, 'r'); % indicates optogenetic stimulation at t=1sec which is equal to frame #20
% title("Neuron #50 - Average with Standard Deviation");
...
% xLimit is 220 for cond1   
% yLimits sometimes also need to be changed but do that directly in the function if needed

% Plot neurons 1 through 5
figure;
plotNeurons(av_neuron*100, 1, 5, "Average Neuron Values Across 5 Stimulation Cycles (SC)", 220);

% Plot neurons 10 through 20
figure;
plotNeurons(av_neuron*100, 10, 20, "Average Neuron Values Across 10 Stimulation Cycles (SC)", 220);
...
av_movie = zeros(size(f,1),size(f,2), 220); % cond1
...
% Set folder and filename where you want to save the video
name = 'AveragedNeuronalActivity.avi';
fullpath = fullfile(current_save_directory, name);
...
time = -1:0.05:9.95; % cond1

roi = squeeze(mean(mean(av_movie,1),2));

open(vidfile);
figure('Position', [10 10 1200 600])
% figure('Position', get(0, 'ScreenSize')) % makes the figure enter full screen
...
subplot(4,20,1:60);
 ...
 % ylim([520 560]); % sometimes you need to manually change this
 xlim([-1 10]);
...
if SAVE_FLAG
    ...
    save(full_path, 'org_idx', 'F_dff', 'AllTiffFiles', 'neurons_stim_cycles', 'stim_start', 'av_neuron', 'av_movie', 'time', 'roi');
    % 'Fall', 'f' are too big to save
