...
addpath(genpath('/home/silviu/erika-organoid-data/CODE/CURRENT-FOLDER/signal-correlation-master')); % contains meta_k_means_cond2.m function
...
for org_idx = 6 % 6:length(organoid_folders) % 1:length(organoid_folders)
...
% trying to return it to 1000 for "run_3" organoids
...
stimulation_interval = 400; % CHANGE: for cond1 every 10 sec (200) and for cond2/3 every 20 sec (400)
total_frames = 12600; % CHANGE: For cond1 (6600) and for cond2/3 (12600)
...
windows_15s = arrayfun(@(s) s+frame_rate*5:min(s+frame_rate*20-1, total_frames), stimulation_points, 'UniformOutput', false);
    % Still need to CHANGE this line to adapt it for cond1
...
%% Meta k means
[allclusters, centroidcorr, dendmem, dunnsinitial] = meta_k_means_cond2(F_dff_dec_segmented', 'correlation');
...
xticks(100:100:length(new_order));
xticklabels(100:100:length(new_order));
...
xticks(100:100:length(random_order));
xticklabels(100:100:length(random_order));
xlabel('Neuron #', 'Color', 'k');
...
if SAVE_FLAG
    ...
    save(full_path, 'allclusters_new', 'neuron_changes', 'remove_columns', 'F_dff_dec', 'F_dff_dec_segmented', 'allclusters', 'centroidcorr', 'dendmem', 'dunnsinitial');
